...
std::pair<std::string, std::vector<std::string>> TIDTree::path_parser(
...
std::string TIDTree::find_var(Node* cur, std::string& name,
    ...
    if (down_type == NodeType::SCOPE || down_type == NodeType::BODY) {
        return find_var(cur->_children[path[position]], name, path,
                        position + 1);
    }
    throw std::invalid_argument("path incorrect");
}
...
std::string TIDTree::find_func(Node* cur, std::string& name,
    ...
    if (down_type == NodeType::SCOPE) {
        return find_func(cur->_children[path[position]], name, param_types,
                         path, position + 1);
    }
    throw std::invalid_argument("path incorrect");
}
...
std::map<std::string, std::string>& TIDTree::find_type(
    Node* cur, std::string name, std::vector<std::string> path,
    int position) const {
    if (position == (int)path.size()) {
    ...
    auto down_type = cur->_children.find(path[position])->second->_type;
    if (down_type == NodeType::SCOPE) {
        return find_type(cur->_children[path[position]], name, path,
                         position + 1);
    }
    throw std::invalid_argument("path incorrect");
}

std::map<std::string, std::string>& TIDTree::find_type(Node* cur,
...
void TIDTree::find_field(std::string& type, std::vector<std::string>& path,
    ...
    if (!fields.count(path[position])) {
        throw std::invalid_argument("Type " + type + " hasn't field " +
                                    path[position]);
    }
...
void TIDTree::erase_var(Node* cur, std::string name,
                        std::vector<std::string> path, int position) {
    if (position == (int)path.size()) {
...
std::map<std::string, std::string> TIDTree::get_type_fields(
    std::string name) const {
...
void TIDTree::push_func(std::string name, std::string return_type,
...
void TIDTree::push_type(std::string name,
...
void TIDTree::push_field(std::string type, std::string name,
                         std::string field_type) {
    auto fields = get_type_fields(type);

    if (fields.count(name)) {
        throw std::invalid_argument("Field " + name +
                                    " already exists in type " + type);
    }
